...
size_t _align_to_8_multiplication(size_t size){
    if (size % 8 == 0)
        return size;
...
void* smalloc(size_t size){
    size = _align_to_8_multiplication(size);
...
void* scalloc(size_t num, size_t size){
    size_t size_num_align = _align_to_8_multiplication(size*num);
    if (size == 0 || size_num_align > pow(10,8))
        return NULL;

    void* new_allocation = smalloc(size_num_align);
    if (new_allocation == NULL)
        return NULL;

    memset(new_allocation, 0, size_num_align);
